...
export const usePokeProgression=create<ProgressionStore>()(persist((set,get)=>({...emptyPokeProgression(),record:(run)=>set((state)=>applyPokeRun(state,run)),spendCredits:(amount)=>{if(!canSpendCredits(get().researchCredits,amount))return false;set((state)=>({...state,researchCredits:state.researchCredits-amount}));return true},addCredits:(amount)=>set((state)=>({...state,researchCredits:state.researchCredits+Math.max(0,amount)})),reset:()=>set(emptyPokeProgression())}),{name:POKE_STORAGE_KEYS.progression,version:3,migrate:(persisted)=>{
 const old=(persisted??{}) as Partial<ProgressionStore>,defaults=emptyPokeProgression();
 return{...defaults,...old,competencyXp:{...defaults.competencyXp,...old.competencyXp}} as ProgressionStore;
}}));
...
}),{name:POKE_STORAGE_KEYS.cards,version:3,migrate:(persisted)=>{
 const old=(persisted??{}) as Partial<CardStore>;
 const collection:CardStore["collection"]={};
 for(const item of Object.values(old.collection??{})){
  try{const card=normalizeLegacyCard(item.card),key=cardVariantKey(card),count=item.count??1;collection[key]=collection[key]?{card,count:collection[key].count+count}:{card,count}}catch{}
 }
 const pending=old.pending?.flatMap((card)=>{try{return[normalizeLegacyCard(card)]}catch{return[]}})??null;
 return{...old,collection,pending} as CardStore;
}}));
