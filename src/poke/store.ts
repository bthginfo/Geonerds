...
export const usePokeProgression=create<ProgressionStore>()(persist((set,get)=>({...emptyPokeProgression(),record:(run)=>set((state)=>applyPokeRun(state,run)),spendCredits:(amount)=>{if(!canSpendCredits(get().researchCredits,amount))return false;set((state)=>({...state,researchCredits:state.researchCredits-amount}));return true},addCredits:(amount)=>set((state)=>({...state,researchCredits:state.researchCredits+Math.max(0,amount)})),reset:()=>set(emptyPokeProgression())}),{name:POKE_STORAGE_KEYS.progression,version:4,migrate:(persisted)=>{
