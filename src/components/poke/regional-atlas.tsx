...
export function RegionalAtlas({
  ...
  const [keyboardCursor, setKeyboardCursor] = useState<AtlasPoint>({ x: 50, y: 50 });
  ...
  const biomeLegend = useMemo(
    () => [...new Set(region.nodes.map((node) => node.biome))].slice(0, 3),
    [region],
  );
  const routes = useMemo(
    () =>
      region.nodes.slice(1).map((node, index) => {
  ...
  const pointFromEvent = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    return {
      x: (rawX - 50 - pan.x) / zoom + 50,
      y: (rawY - 50 - pan.y) / zoom + 50,
    };
  };
  ...
  const pointerMove = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (pointers.current.size === 2) {
      ...
      if (gesture.current.lastDistance > 0)
        applyZoom(zoom * (distance / gesture.current.lastDistance));
    ...
    if (zoom > 1) {
      ...
      setPan((value) => ({
        x: Math.max(
          ...
          Math.min(
            36,
            value.x + ((event.clientX - previous.x) / rect.width) * 100,
          ),
        ),
        y: Math.max(
          -36,
          Math.min(
            36,
            value.y + ((event.clientY - previous.y) / rect.height) * 100,
          ),
        ),
      }));
    }
  ...
  const keyboardPlace = (event: React.KeyboardEvent<SVGSVGElement>) => {
    if (!onPlace) return;
    const movement: Record<string, AtlasPoint> = {
      ArrowLeft: { x: -3, y: 0 },
      ArrowRight: { x: 3, y: 0 },
      ArrowUp: { x: 0, y: -3 },
      ArrowDown: { x: 0, y: 3 },
    ...
    if (delta) {
      ...
      setKeyboardCursor((point) => ({
        x: Math.max(2, Math.min(98, point.x + delta.x)),
        y: Math.max(2, Math.min(98, point.y + delta.y)),
      }));
    } else if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      setKeyboardActive(true);
      onPlace(keyboardCursor);
    }
  };

  const interactionHint = onPlace
    ? locale === "de"
      ? "Tippen oder fokussieren: Pfeile bewegen · Enter setzt"
      : "Tap or focus: arrows move · Enter places"
    : onSector
      ? locale === "de"
        ? "Sektoren antippen: Mikroklima scannen"
        : "Tap sectors to scan microclimate"
      : locale === "de"
        ? "Ziehen, scrollen oder pinch-to-zoom"
        : "Drag, scroll or pinch to zoom";

  return (
    <figure
      className={`poke-atlas poke-regional-atlas ${fog ? "has-fog" : ""} ${placement ? "has-placement" : ""} ${concealBiomes ? "has-concealed-biomes" : ""}`}
      style={
        {
    ...
    >
      <header className="poke-atlas-header">
        <div>
          <span>
            {locale === "de" ? "REGIONSATLAS" : "REGIONAL ATLAS"} · GEN{" "}
            {region.generation}
          </span>
          <b>{region.name[locale]}</b>
        </div>
        <div className="poke-atlas-zoom-readout">
          <span>MAG</span>
          <b>{zoom.toFixed(1)}×</b>
        </div>
      </header>
      <div
        className="poke-atlas-tools"
        aria-label={locale === "de" ? "Kartensteuerung" : "Map controls"}
      >
        <button
          type="button"
        ...
        <button
          ...
          aria-label={locale === "de" ? "Karte zurücksetzen" : "Reset map"}
        >
          <RotateCcw />
        </button>
      </div>
      <svg
        ...
        onFocus={() => onPlace && setKeyboardActive(true)}
        onKeyDown={keyboardPlace}
        aria-label={`${region.name[locale]} · ${
          onPlace
            ? locale === "de"
              ? "Interaktive Karte. Pfeiltasten bewegen das Fadenkreuz; Enter oder Leertaste setzt die Position."
              : "Interactive map. Arrow keys move the targeting cursor; Enter or Space places the position."
            : locale === "de"
      ...
      >
        ...
        <g
          transform={`translate(${pan.x} ${pan.y}) translate(50 50) scale(${zoom}) translate(-50 -50)`}
        >
          <path
            className="poke-region-coast-glow"
            d={region.path}
            fill="none"
            stroke={region.accent}
            filter={`url(#${defs.glow})`}
          />
          <path
            className="poke-region-land"
            d={region.path}
            fill={`url(#${defs.land})`}
            stroke={region.accent}
          />
          <g
            clipPath={`url(#${defs.clip})`}
            className="poke-atlas-land-detail"
            aria-hidden="true"
          >
            ...
            {region.nodes.map((node) => (
              <circle
                key={`terrain-${node.id}`}
                className={`poke-biome-pocket ${concealBiomes ? "biome-unknown" : `biome-${node.biome}`}`}
                cx={node.x}
                cy={node.y}
                r="8.5"
              />
          ...
          {placement && activeNode && showRings && (
            <line
              className="poke-survey-vector"
              x1={placement.x}
              y1={placement.y}
              x2={activeNode.x}
              y2={activeNode.y}
          ...
          {region.nodes.map((node) => (
            <g
              ...
              aria-hidden={
                !onSector && !showLabels && !showRings ? true : undefined
              }
              onKeyDown={(event) => {
                if (onSector && (event.key === "Enter" || event.key === " ")) {
                  event.preventDefault();
                  onSector(node.id);
                }
              }}
            ...
            >
              ...
              {showRings && activeSector === node.id && (
                <>
                  <circle className="poke-distance-ring" r="7" />
                  <circle className="poke-distance-ring is-wide" r="13" />
                  <text className="poke-map-resolved-label" x="4" y="-4">
                    {node.name[locale]}
                  </text>
                </>
              )}
          ...
          {placement && (
            <g
              className="poke-survey-pin"
              transform={`translate(${placement.x} ${placement.y})`}
              aria-label={
                locale === "de" ? "Dein Messpunkt" : "Your survey pin"
              }
            >
          ...
          {probes.map((probe, index) => (
            <g
              key={`${probe.x}-${probe.y}-${index}`}
              className="poke-probe-ring"
          ...
          {onPlace && (
            <g
              className={`poke-keyboard-cursor ${keyboardActive ? "is-active" : ""}`}
              transform={`translate(${keyboardCursor.x} ${keyboardCursor.y})`}
              aria-hidden="true"
            >
              <circle r="4.5" />
        ...
        {fog && (
          <g className="poke-atlas-fog" pointerEvents="none" aria-hidden="true">
        ...
        <g className="poke-coordinate-ticks" aria-hidden="true">
          {[10, 30, 50, 70, 90].map((value) => (
            <g key={`tick-${value}`}>
              <path
                d={`M${value} 0v1.6M${value} 100v-1.6M0 ${value}h1.6M100 ${value}h-1.6`}
              />
              <text x={value + 0.8} y="3.4">
                {value}
              </text>
            </g>
          ))}
      ...
      <div className="poke-atlas-interaction">
        <LocateFixed />
        <span>{interactionHint}</span>
      </div>
      <div
        className="poke-atlas-legend"
        aria-label={locale === "de" ? "Kartenlegende" : "Map legend"}
      >
        ...
        {concealBiomes ? (
          <span className="biome-unknown">
            <i />
            {locale === "de" ? "Unbekannte Sektoren" : "Unknown sectors"}
          </span>
        ) : (
          biomeLegend.map((biome) => (
            <span key={biome} className={`biome-${biome}`}>
              <i />
              {BIOME_LABELS[biome][locale]}
            </span>
          ))
        )}
        {placement && (
          <span className="is-pin">
            <i />
            {locale === "de" ? "Dein Pin" : "Your pin"}
          </span>
        )}
      </div>
      <div
        className="poke-map-pan"
        aria-label={locale === "de" ? "Karte verschieben" : "Pan map"}
      >
        <button
          type="button"
          disabled={zoom <= 1}
          onClick={() => setPan((p) => ({ ...p, x: Math.min(36, p.x + 5) }))}
          aria-label={locale === "de" ? "Nach links verschieben" : "Pan left"}
        >
          ←
        </button>
        <button
          type="button"
          disabled={zoom <= 1}
          onClick={() => setPan((p) => ({ ...p, y: Math.min(36, p.y + 5) }))}
          aria-label={locale === "de" ? "Nach oben verschieben" : "Pan up"}
        >
          ↑
        </button>
        <button
          type="button"
          disabled={zoom <= 1}
          onClick={() => setPan((p) => ({ ...p, y: Math.max(-36, p.y - 5) }))}
          aria-label={locale === "de" ? "Nach unten verschieben" : "Pan down"}
        >
          ↓
        </button>
        <button
          type="button"
          disabled={zoom <= 1}
          onClick={() => setPan((p) => ({ ...p, x: Math.max(-36, p.x - 5) }))}
          aria-label={locale === "de" ? "Nach rechts verschieben" : "Pan right"}
        >
          →
        </button>
      </div>
      <figcaption className="poke-map-caption">
        {locale === "de"
          ? "Originale schematische Lernkarte · nicht maßstabsgetreu"
          : "Original schematic learning map · not to scale"}
      </figcaption>
    </figure>
  );
}
