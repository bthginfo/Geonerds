...
import {ArrowLeft,Check,CircleHelp,RotateCcw,Sparkles} from "lucide-react";
...
const gameHelp:Record<PokeGameId,{prompt:{en:string;de:string};rule:{en:string;de:string}}>={
 "guess-that-pokemon":{prompt:{en:"Identify the artwork, then lock one answer.",de:"Erkenne das Artwork und verriegle dann eine Antwort."},rule:{en:"Hard mode uses exact text input; hints reduce uncertainty but not the timer.",de:"Im Hard Mode tippst du den exakten Namen; Hinweise reduzieren die Unsicherheit, nicht den Timer."}},
 "poke-path-expedition":{prompt:{en:"Read the route forecast before choosing the next sector.",de:"Lies die Routenprognose, bevor du den nächsten Sektor wählst."},rule:{en:"Your whole team changes route damage and rewards; capsules can heal or recruit.",de:"Dein gesamtes Team verändert Routenschaden und Ertrag; Kapseln heilen oder rekrutieren."}},
 "region-ranger":{prompt:{en:"Tap the atlas where you believe the named sector belongs.",de:"Tippe im Atlas dorthin, wo du den genannten Sektor vermutest."},rule:{en:"The map never zooms to the answer. Optional scans cost 100 potential points each.",de:"Die Karte zoomt nie zur Antwort. Optionale Scans kosten je 100 mögliche Punkte."}},
 "habitat-hunt":{prompt:{en:"Scan a sector, compare its three traits, then lock a match.",de:"Scanne einen Sektor, vergleiche drei Merkmale und lege dann einen Treffer fest."},rule:{en:"Only scanned sectors can be locked, and every extra scan lowers the reward.",de:"Nur gescannte Sektoren lassen sich festlegen; jeder weitere Scan senkt den Ertrag."}},
 "type-clash-arena":{prompt:{en:"Choose the move with the strongest projected matchup.",de:"Wähle die Attacke mit der stärksten prognostizierten Wirkung."},rule:{en:"STAB, type effectiveness, power and energy all affect the final line.",de:"STAB, Typenwirkung, Stärke und Energie beeinflussen die finale Linie."}},
 "gym-draft-gauntlet":{prompt:{en:"Draft six Pokémon without exceeding the field budget.",de:"Drafte sechs Pokémon, ohne das Feldbudget zu überschreiten."},rule:{en:"Diverse type coverage matters more than stacking one powerful specialty.",de:"Breite Typen-Coverage zählt mehr als eine einzige starke Spezialisierung."}},
 "evolution-lab":{prompt:{en:"Place each species in family order, then set every trigger.",de:"Ordne jede Spezies in Familienfolge an und setze dann alle Auslöser."},rule:{en:"Tap a placed species to reposition it; branch conditions must match their edge.",de:"Tippe ein platziertes Pokémon zum Versetzen an; Verzweigungen müssen zur Kante passen."}},
 "binder-ascension":{prompt:{en:"Read the enemy intent, then choose a route or card line.",de:"Lies die Gegnerabsicht und wähle dann Route oder Kartenlinie."},rule:{en:"Your binder deck persists through the climb, so protect scarce healing and upgrades.",de:"Dein Binder-Deck bleibt über den Aufstieg bestehen – schütze knappe Heilung und Upgrades."}},
 "field-capture":{prompt:{en:"Calibrate, inspect gear, then drag and release the ball.",de:"Kalibrieren, Ausrüstung prüfen, dann den Ball ziehen und loslassen."},rule:{en:"A smaller catch ring and accurate upward release improve quality; rarity still matters.",de:"Ein kleinerer Fangring und präzises Loslassen nach oben verbessern die Qualität; Seltenheit zählt weiterhin."}},
 "field-scanner":{prompt:{en:"Read the silhouette and spend only the hints you need.",de:"Lies die Silhouette und nutze nur die Hinweise, die du brauchst."},rule:{en:"Candidate names stay visible, but artwork is revealed only after resolution.",de:"Kandidatennamen bleiben sichtbar; das Artwork erscheint erst nach der Auflösung."}},
 "cry-radar":{prompt:{en:"Play the signal once, compare its spectrum, then answer.",de:"Spiele das Signal einmal ab, vergleiche das Spektrum und antworte dann."},rule:{en:"Audio never autoplays; replay and waveform details are part of the deduction.",de:"Audio startet nie automatisch; Wiederholung und Wellenform gehören zur Deduktion."}},
 "poke-grid":{prompt:{en:"Fill an intersection with one species matching both rules.",de:"Fülle eine Schnittmenge mit einer Spezies, die beide Regeln erfüllt."},rule:{en:"A species cannot be reused within the same grid; research assists cost score.",de:"Eine Spezies darf im selben Grid nicht doppelt vorkommen; Forschungshilfen kosten Punkte."}},
 "professor-case-files":{prompt:{en:"Read the current clue and cross out only contradicted suspects.",de:"Lies die aktuelle Spur und streiche nur widersprochene Verdächtige."},rule:{en:"Reveal the next sequential clue for a score cost, then lock a dossier when the visible evidence is sufficient.",de:"Decke die nächste Spur gegen Punktabzug auf und lege eine Akte fest, sobald die sichtbaren Beweise ausreichen."}},
};
