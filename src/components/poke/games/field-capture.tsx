...
export function FieldCapture({locale,difficulty,generationCap,roundCount,runSeed,onFinish}:GameProps){
 ...
 const beginEncounter=()=>{
  ...
  const attempts=3+(earned==="extra"?1:0);setAttemptsLeft(attempts);setMaxAttempts(attempts);setBall("field");setBerry(false);setResult(null);setFlight("ready");setDrag({active:false,start:null,points:[]});setPhase("encounter");
 };
 const beginDrag=(event:React.PointerEvent<HTMLDivElement>)=>{
  if(result||flight!=="ready"||!(event.target as HTMLElement).closest("[data-capture-cradle]"))return;
 ...
 const fallbackThrow=()=>performThrow({accuracy:Math.max(0,1-Math.abs(aim-50)/50),ring,direction:.82,speed:.35,curve:false},{x:aim,y:48});
 ...
 const advance=()=>{
  ...
  setRound((value)=>value+1);setPrepResult(null);setResult(null);setActivePrep(null);setFlight("ready");setScanPos(0);setPhase("prep");
 };
