...
function RouteCard({route,deck,locale,encounter,difficulty,runSeed,onChoose}:{route:AscensionRoute;deck:PokeCard[];locale:"en"|"de";encounter:number;difficulty:PokeDifficulty;runSeed:string;onChoose:()=>void}){
 const intent=enemyIntent(route,encounter,1,difficulty,runSeed),hp=enemyMaxHp(route,encounter,difficulty);
 const best=deck.map((card)=>{const entry=species(card.speciesId),damage=Math.round(cardBaseDamage(entry)*typeMultiplier(entry.types[0],route.opponent.types));return{entry,damage}}).sort((a,b)=>b.damage-a.damage)[0];
 const projectedTurns=best?Math.max(1,Math.ceil(hp/Math.max(1,best.damage))):0;
 return <button className={`poke-route-card is-${route.kind}`} onClick={onChoose} aria-label={`${routeName(route.kind,locale)} · ${route.opponent.name[locale]}`}><header><span>{routeIcon(route.kind)} {routeName(route.kind,locale).toUpperCase()}</span><b>{Array.from({length:5},(_,index)=><i className={index<route.threat?"is-on":""} key={index}/>)}</b></header><div className="poke-route-opponent"><PokemonSprite entry={route.opponent} size={150}/><span><small>#{String(route.opponent.id).padStart(4,"0")}</small><strong>{route.opponent.name[locale]}</strong><em>{route.opponent.types.map((type)=>localizedType(type,locale)).join(" · ")}</em></span></div><dl><div><dt>HP</dt><dd>{hp}</dd></div><div><dt>{locale==="de"?"ERSTE ABSICHT":"FIRST INTENT"}</dt><dd>{intent.damage} DMG</dd></div><div><dt>{locale==="de"?"BELOHNUNG":"REWARD"}</dt><dd>+{route.reward}</dd></div></dl><p>{routeDescription(route.kind,locale)}{best&&<><br/><b>{locale==="de"?"Deck-Read":"Deck read"}: {best.entry.name[locale]} · {best.damage} DMG · ~{projectedTurns} {locale==="de"?"Züge":"turns"}</b></>}</p><footer>{locale==="de"?"Pfad verriegeln":"Lock route"} <ChevronRight/></footer></button>;
}
