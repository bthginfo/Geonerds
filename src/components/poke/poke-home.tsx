...
import {ArrowRight,AudioWaveform,FileSearch,GitBranch,Grid3X3,Layers3,MapPinned,PawPrint,Radar,Route,ScanSearch,Swords,UsersRound} from "lucide-react";
...
const marks=[ScanSearch,Route,MapPinned,PawPrint,Swords,UsersRound,GitBranch,Layers3,Radar,ScanSearch,AudioWaveform,Grid3X3,FileSearch];
export function PokeHome(){
 ...
 return <div className="poke-home">
  <section className="poke-hero">
   <div className="poke-hero-copy"><p className="poke-kicker">FIELD RESEARCH OS // KANTO-01</p><h1>{locale==="de"?"Beobachten. Kombinieren. Meistern.":"Observe. Connect. Master."}</h1><p>{locale==="de"?"Dreizehn taktile Pokémon-Trainingsmissionen auf einem lebendigen Forschungstisch. Keine Quiztapete – echte Karten, Scanner, Labore und Teamentscheidungen.":"Thirteen tactile Pokémon training missions on a living research table. No quiz wallpaper—real maps, scanners, labs and team decisions."}</p><Link className="poke-primary" href={`/poke-nerds/play/${recommended.id}`}>{locale==="de"?"Empfohlene Mission starten":"Start recommended mission"} <ArrowRight/></Link></div>
  ...
  <section className="poke-mission-index">
   <header><div><p className="poke-kicker">13 ACTIVE MODULES</p><h2>{locale==="de"?"Missionsverzeichnis":"Mission index"}</h2></div><p>{locale==="de"?"Jede Mission trainiert eine eigene Feldkompetenz.":"Each mission trains a distinct field competency."}</p></header>
